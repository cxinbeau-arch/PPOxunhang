...
def cell_info(step: Dict[str, Any], x: int, y: int) -> Dict[str, Any]:
    ...
    agent_tuple = tuple(agent) if isinstance(agent, (list, tuple)) and len(agent) >= 2 else None
    base_tuple = tuple(base) if isinstance(base, (list, tuple)) and len(base) >= 2 else None
    ...
    return {
        ...
        "is_base": base_tuple is not None and p == base_tuple,
        "is_agent_current": agent_tuple is not None and p == agent_tuple,
    }
